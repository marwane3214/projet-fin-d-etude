...
export default function LiquidationListPage() {
  ...
  // ═══════════════════════════════════════════════════════
  if (!isAdmin) {
    const myDemande = demandes.find(d => d.affilieId === user?.affilieId);
    const hasSubmitted = !!myDemande || localStorage.getItem(`cimr_liquidation_submitted_${user?.affilieId ?? user?.username}`) === 'true';
